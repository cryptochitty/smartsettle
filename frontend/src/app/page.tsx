...
function NegotiationModal({ bill, onClose }) {
  ...
  return (
    <div
      ...
      style={{ position:"fixed",inset:0,zIndex:100,background:"rgba(2,6,18,0.88)",backdropFilter:"blur(12px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20 }}
    >
      <div style={{ width:"100%",maxWidth:500,maxHeight:"88vh",overflowY:"auto",background:"#060e1f",border:"1px solid #1e3354",borderRadius:24,animation:"popIn .3s cubic-bezier(.34,1.56,.64,1)" }}>
...
export default function App() {
  ...
  // ✅ Fixed: proper HTMLInputElement ref
      ...
      <header style={{ position:"sticky",top:0,zIndex:50,background:"rgba(3,9,18,0.85)",backdropFilter:"blur(20px)",borderBottom:"1px solid #0f1e35",height:62,padding:"0 28px",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
        <div style={{ display:"flex",alignItems:"center",gap:10 }}>
          <div style={{ width:42,height:42,borderRadius:12,overflow:"hidden",flexShrink:0,background:"#0d1929",border:"1px solid #1e3354",display:"flex",alignItems:"center",justifyContent:"center" }}>
            {!logoError
              ? <img src={LOGO_URL} alt="SmartSettle" onError={() => setLogoError(true)} style={{ width:42,height:42,objectFit:"contain",borderRadius:12 }} />
        ...
        <nav style={{ display:"flex",gap:2 }}>
          {["dashboard","bills","receipts","wallet"].map(t => (
            <button key={t} onClick={() => setTab(t)} style={{ background:tab===t?"#00ff8710":"transparent",border:`1px solid ${tab===t?"#00ff8732":"transparent"}`,borderRadius:8,padding:"6px 14px",fontSize:11,fontWeight:700,color:tab===t?"#00ff87":"#2a4878",cursor:"pointer",letterSpacing:"0.1em",transition:"all .15s" }}>
              {t.toUpperCase()}
            </button>
          ))}
        </nav>
        ...
        <button onClick={() => setConn(!connected)} style={{ background:connected?"#00ff8712":"linear-gradient(135deg,#00ff87,#38bdf8)",border:connected?"1px solid #00ff8730":"none",borderRadius:10,padding:"8px 18px",fontSize:11,fontWeight:800,color:connected?"#00ff87":"#020c1c",cursor:"pointer",letterSpacing:"0.06em",animation:connected?"none":"glow 2.5s infinite",transition:"all .2s" }}>
      ...
      <main style={{ maxWidth:1080,margin:"0 auto",padding:"36px 24px",position:"relative",zIndex:1 }}>
        ...
        {!connected && (
          <div style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"72vh",textAlign:"center",gap:28,animation:"slideUp .5s ease" }}>
            <div style={{ animation:"float 3s ease-in-out infinite" }}>
              {!logoError
                ? <img src={LOGO_URL} alt="SmartSettle" onError={() => setLogoError(true)} style={{ width:140,height:140,objectFit:"contain",borderRadius:28,filter:"drop-shadow(0 0 30px #00ff8740)" }} />
            ...
            <button onClick={() => setConn(true)} style={{ background:"linear-gradient(135deg,#00ff87,#38bdf8)",border:"none",borderRadius:14,padding:"16px 44px",fontSize:14,fontWeight:900,color:"#020c1c",cursor:"pointer",letterSpacing:"0.06em",animation:"glow 2.5s infinite" }}>
              CONNECT WALLET →
            </button>
          ...
          <div style={{ animation:"slideUp .4s ease" }}>
            ...
            <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,marginBottom:28 }}>
              {[
                ...
                <div key={i} style={{ background:"linear-gradient(135deg,#060e1f,#04091a)",border:`1px solid ${s.color}22`,borderRadius:20,padding:"22px 20px",position:"relative",overflow:"hidden",animation:"slideUp .4s ease",animationDelay:`${i*70}ms` }}>
              ...
              <div style={{ background:"linear-gradient(135deg,#060e1f,#04091a)",border:"1px solid #0f1e35",borderRadius:20,padding:24 }}>
                ...
                <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
                  {BILLS.map((b, i) => {
                    ...
                    return (
                      <div key={b.id}
                        style={{ display:"flex",alignItems:"center",gap:12,background:"#04091a",border:"1px solid #0f1e35",borderRadius:14,padding:"12px 14px",transition:"all .18s",cursor:b.status==="pending"?"pointer":"default",animation:"slideUp .35s ease",animationDelay:`${i*55}ms` }}
                      ...
                      >
                        ...
                        {b.status === "pending" && (
                          <button onClick={() => setSettling(b)} style={{ background:"linear-gradient(135deg,#00ff87,#38bdf8)",border:"none",borderRadius:8,padding:"7px 12px",fontSize:10,fontWeight:800,color:"#020c1c",cursor:"pointer",flexShrink:0 }}>SETTLE →</button>
                        )}
                      </div>
                    );
                  })}
        ...
        {connected && tab === "bills" && (
          <div style={{ animation:"slideUp .4s ease" }}>
            ...
            {/* ✅ Fixed: fileRef.current check */}
            <div
              onDragOver={e => { e.preventDefault(); setDrag(true); }}
              onDragLeave={() => setDrag(false)}
              onDrop={e => { e.preventDefault(); setDrag(false); setSettling(BILLS[0]); setTab("dashboard"); }}
              onClick={() => { if (fileRef.current) fileRef.current.click(); }}
              style={{ border:`2px dashed ${drag?"#00ff87":"#0f1e35"}`,borderRadius:22,padding:"56px 40px",textAlign:"center",cursor:"pointer",marginBottom:28,transition:"all .22s",background:drag?"#00ff8706":"linear-gradient(135deg,#060e1f,#04091a)" }}
            >
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
              {BILLS.map((b, i) => {
                ...
                return (
                  <div key={b.id}
                    style={{ display:"flex",alignItems:"center",gap:14,background:"linear-gradient(135deg,#060e1f,#04091a)",border:"1px solid #0f1e35",borderRadius:16,padding:"15px 20px",transition:"all .18s",animation:"slideUp .35s ease",animationDelay:`${i*55}ms` }}
                  ...
                  >
                    ...
                    {b.status === "pending" && (
                      <button onClick={() => setSettling(b)} style={{ background:"linear-gradient(135deg,#00ff87,#38bdf8)",border:"none",borderRadius:10,padding:"9px 18px",fontSize:11,fontWeight:800,color:"#020c1c",cursor:"pointer" }}>AUTO-SETTLE</button>
                    )}
                  </div>
                );
              })}
          ...
          <div style={{ animation:"slideUp .4s ease",maxWidth:500 }}>
            ...
            <div style={{ background:"linear-gradient(135deg,#060e1f,#04091a)",border:"1px solid #0f1e35",borderRadius:20,padding:24 }}>
              ...
              <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:16,borderBottom:"1px solid #0f1e35",paddingBottom:16 }}>
                ...
                <input type="number" placeholder="0.00" style={{ flex:1,background:"none",border:"none",outline:"none",fontSize:34,fontWeight:900,color:"#d8eeff" }} />
              </div>
