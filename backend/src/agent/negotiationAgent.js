const { complete } = require("../services/llmAdapter");
...
async function selectBestOffer(providerName, originalAmount, offers) {
  const raw = await complete(
    `You are a bill negotiation agent. Pick the best offer for the user.
...
{ "selectedIndex": number, "reasoning": string }`,
  ...
  const pick = JSON.parse(raw.replace(/```json|```/g, "").trim());
  // Prompt uses 1-based numbering, convert to 0-based index
  return { offer: offers[pick.selectedIndex - 1], reasoning: pick.reasoning };
}
